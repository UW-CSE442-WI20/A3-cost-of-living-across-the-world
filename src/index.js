...
(function() {
  ...
  function updateMap() {
    ...
    var circle = g.selectAll("circle")
        ...
        .attr("cx", function(d){ return projection([+d.Longitude, +d.Latitude])[0] }) // coordinates
        .attr("cy", function(d){ return projection([+d.Longitude, +d.Latitude])[1] })
        .style("fill", function(d){console.log('hello'); return scaleCircles(d)}) // scale value to color
        .attr("fill-opacity", .7)
        .attr("stroke", "black")
        .attr("stroke-width", 0.2)
        .on("mouseover", mouseover) // function calls for tooltip
  ...
  function scaleCircles(d) {
    ...
    for (var i = 0; i < subs.length; i++) {
      if (subs[i].checked) {
        filteredTotal += (+d.subs[i].innerHTML);
        console.log((+d.subs[i].innerHTML));
      }
    }
    return colorScale(filteredTotal);
  }
